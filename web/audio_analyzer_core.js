...
export class AudioAnalyzerInterface {
    ...
    async loadCachedData() {
        ...
        if (!this.node?.id || this.node.id < 0) {
    ...
    pixelToTime(pixel) {
        ...
        const canvasWidth = this.getCanvasLogicalWidth();
    ...
    getCanvasCoordinates(clientX, clientY) {
        ...
        const logicalCanvasWidth = this.getCanvasLogicalWidth();
        const logicalCanvasHeight = this.getCanvasLogicalHeight();
    ...
    getCanvasPixelScale() {
        ...
        const zoom = Math.max(0.1, Math.min(4, this.getComfyUIZoomLevel() || 1));
        return Math.max(1, Math.min(4, dpr * zoom));
    }

    getCanvasCssWidth() {
        if (!this.canvas) return 0;
        return this.canvas.clientWidth || this.canvas.offsetWidth || 0;
    }

    getCanvasCssHeight() {
        if (!this.canvas) return 0;
        return this.canvas.clientHeight || this.canvas.offsetHeight || 0;
    }

    getCanvasLogicalWidth() {
        if (!this.canvas) return 0;
        return this.getCanvasCssWidth() || (this.canvas.width / this.getCanvasPixelScale());
    }

    getCanvasLogicalHeight() {
        if (!this.canvas) return 0;
        return this.getCanvasCssHeight() || (this.canvas.height / this.getCanvasPixelScale());
    }
    ...
    resizeCanvas(redraw = true) {
        const cssWidth = this.getCanvasCssWidth();
        const cssHeight = this.getCanvasCssHeight();
        if (!cssWidth || !cssHeight) return false;

        const pixelScale = this.getCanvasPixelScale();
