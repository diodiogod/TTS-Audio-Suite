...
This module provides functionality to composite edited audio segments with original audio,
preserving quality in non-edited regions while applying smooth crossfades at boundaries.
"""
...
class AudioCompositor:
    ...
    def composite_edited_audio(original_audio: torch.Tensor, generated_audio: torch.Tensor, 
        ...
        Args:
            original_audio: Original audio tensor [1, samples]
            generated_audio: F5-TTS generated audio tensor [1, samples]
            edit_regions: List of (start_time, end_time) tuples in seconds
            sample_rate: Audio sample rate
            crossfade_ms: Crossfade duration in milliseconds for smooth transitions
            
        Returns:
            Composited audio tensor with original quality preserved outside edit regions
        """
        ...
        original_audio = AudioCompositor._pad_audio_to_length(original_audio, max_length)
        generated_audio = AudioCompositor._pad_audio_to_length(generated_audio, max_length)
        ...
        for start_time, end_time in edit_regions:
            ...
            if crossfade_samples > 0:
                AudioCompositor._apply_boundary_crossfades(
                    composite_audio, generated_audio, start_sample, end_sample, 
                    crossfade_samples, max_length
                )
    ...
    @staticmethod
    def _pad_audio_to_length(audio: torch.Tensor, target_length: int) -> torch.Tensor:
        """Pad audio tensor to target length with zeros."""
        if audio.shape[-1] < target_length:
            padding = target_length - audio.shape[-1]
            audio = torch.cat([audio, torch.zeros(1, padding, device=audio.device)], dim=-1)
        return audio
    
    @staticmethod
    def _apply_boundary_crossfades(composite_audio: torch.Tensor, generated_audio: torch.Tensor,
                                  start_sample: int, end_sample: int, crossfade_samples: int,
                                  max_length: int) -> None:
        """Apply crossfade transitions at edit region boundaries."""
        # Crossfade at start
        if start_sample > 0:
            AudioCompositor._apply_start_crossfade(
                composite_audio, generated_audio, start_sample, crossfade_samples
            )
        
        # Crossfade at end
        if end_sample < max_length:
            AudioCompositor._apply_end_crossfade(
                composite_audio, generated_audio, end_sample, crossfade_samples, max_length
            )
    
    @staticmethod
    def _apply_start_crossfade(composite_audio: torch.Tensor, generated_audio: torch.Tensor,
                              start_sample: int, crossfade_samples: int) -> None:
        """Apply crossfade at the start of an edit region."""
        ...
        if fade_length > 0:
            ...
            if fade_start < generated_audio.shape[-1]:
                ...
                composite_audio[:, fade_start:gen_fade_end] += (
                    generated_audio[:, fade_start:gen_fade_end] * fade_in[:gen_fade_end-fade_start]
                )
    
    @staticmethod
    def _apply_end_crossfade(composite_audio: torch.Tensor, generated_audio: torch.Tensor,
                            end_sample: int, crossfade_samples: int, max_length: int) -> None:
        """Apply crossfade at the end of an edit region."""
        ...
        if fade_length > 0:
            ...
            if end_sample < generated_audio.shape[-1]:
                ...
                composite_audio[:, end_sample:gen_fade_end] += (
                    generated_audio[:, gen_fade_start:gen_fade_end] * fade_in[:gen_fade_end-end_sample]
                )
...
class EditMaskGenerator:
    ...
    def create_edit_mask_and_audio(original_audio: torch.Tensor, edit_regions: List[Tuple[float, float]], 
                                  fix_durations: List[float], target_sample_rate: int, 
        ...
        Args:
            ...
            edit_regions: List of (start_time, end_time) tuples in seconds
            fix_durations: Optional list of fixed durations for edit regions
            target_sample_rate: Target sample rate
            hop_length: Hop length for mel spectrogram
            f5tts_sample_rate: F5-TTS expected sample rate
            
        Returns:
            Tuple of (edited_audio, edit_mask)
        """
        if original_audio.dim() > 1:
            original_audio = torch.mean(original_audio, dim=0, keepdim=True)
