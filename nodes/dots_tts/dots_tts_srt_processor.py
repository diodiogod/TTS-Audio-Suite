"""
Dots TTS SRT processor.

Handles full SRT subtitle processing for Dots TTS.
"""
...
class DotsTTSSRTProcessor:
    """Complete SRT processor for Dots TTS."""
    ...
    def _load_srt_modules(self):
        try:
            ...
            if not success:
                print(f"⚠️ Dots TTS SRT: SRT module not available: {msg}")
                return
            ...
            if self.SRTParser is None:
                print("⚠️ Dots TTS SRT: SRT parser not available")
                return
            self.srt_available = True
        except Exception as e:
            print(f"⚠️ Dots TTS SRT: Failed to load SRT modules: {e}")

    @property
    def processor(self):
        if self._tts_processor is None:
            processor_path = os.path.join(current_dir, "dots_tts_processor.py")
            spec = importlib.util.spec_from_file_location("dots_tts_processor_module", processor_path)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            self._tts_processor = module.DotsTTSProcessor(self.adapter, self.config)
    ...
    def process_srt_content(
        ...
        current_timing_mode, mode_switched = SRTOverlapHandler.handle_smart_natural_fallback(
            timing_mode, has_overlaps, "Dots TTS SRT"
        )

        print(f"🚀 Dots TTS SRT: Processing {len(subtitles)} subtitles in {current_timing_mode} mode")
        ...
        info = (
            f"Generated {total_duration:.1f}s Dots TTS SRT-timed audio from {len(subtitles)} subtitles "
            ...
            if subtitle_index is not None and total_subtitles is not None:
                if character:
                    raise InterruptedError(
                        f"Dots TTS SRT generation interrupted at subtitle "
                        f"{subtitle_index + 1}/{total_subtitles}, character '{character}'"
                    )
                raise InterruptedError(
                    f"Dots TTS SRT generation interrupted at subtitle {subtitle_index + 1}/{total_subtitles}"
                )
            raise InterruptedError("Dots TTS SRT generation interrupted by user")
    ...
    def _process_all_subtitles(
        ...
        for i, sub in enumerate(subtitles):
            ...
            if not subtitle_has_edit_tags:
                ...
                print(f"✅ Dots TTS Subtitle {i + 1}/{len(subtitles)}: {natural_duration:.2f}s (expected {target_duration:.2f}s)")
            else:
                print(f"✅ Dots TTS Subtitle {i + 1}/{len(subtitles)}: queued for batch edit processing")
